...
func (h *UsageHandler) ListSessions(c *gin.Context) {
	...
	items, err := h.usageService.ListWorkSessions(c.Request.Context(), userID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UsageHandler) PatchSession(c *gin.Context) {
	...
	if err := h.usageService.PatchWorkSession(c.Request.Context(), id, 0, service.WorkSessionPatch{
		Title:             body.Title,
...
func (h *UsageHandler) ListSessionRequests(c *gin.Context) {
	...
	items, err := h.usageService.ListWorkSessionRequests(c.Request.Context(), id, 0)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
