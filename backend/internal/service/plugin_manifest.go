...
type PluginRequirements struct {
	...
	RecommendedBerthVersion string   `json:"recommended_berth_version,omitempty"`
	TestedBerthVersions     []string `json:"tested_berth_versions,omitempty"`
...
type PluginCompatibility struct {
	...
	CurrentBerth     string `json:"current_berth_version"`
	RequiredBerth    string `json:"required_berth_version"`
	RecommendedBerth string `json:"recommended_berth_version"`
	PluginProtocol     int    `json:"plugin_protocol"`
	TransportAPI       int    `json:"transport_api"`
	UIBridge           int    `json:"ui_bridge"`
}
...
func (m PluginManifest) Validate() error {
	...
	if strings.TrimSpace(m.Requires.Berth) == "" {
		return errors.New("插件必须声明 requires.berth")
	}
