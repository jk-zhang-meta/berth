...
const (
	...
	// unsupported 账号的重探间隔倍数：上游不是 berth 中转就不会突然长出
	// /v1/berth/billing，按常规 interval 重排只会持续占满每周期
...
// UpstreamBillingProbeService discovers a remote Berth billing snapshot.
...
func (s *UpstreamBillingProbeService) probeLoadedAccount(ctx context.Context, account *Account, intervalMinutes int) (*UpstreamBillingProbeSnapshot, error) {
	...
	if account.Platform == PlatformOpenAI {
		...
		// 必无 /v1/berth/billing；不发请求，直接记 unsupported，避免
	...
	probeURL := buildOpenAIEndpointURL(normalizedBaseURL, "/v1/berth/billing")
...
func parseUpstreamBillingProbeResponse(body []byte) (map[string]any, error) {
	...
	if response.Object != "berth.key_billing" || response.SchemaVersion != 1 || response.BillingScope != "token" {
...
// in to the upstream billing probe. `/v1/berth/billing` is a key-scoped
// berth convention shared by the supported API-key platforms (including the
// CN providers, whose official-domain accounts are short-circuited to
// "unsupported" by upstreamBillingProbeTargetIsOfficialAPI).
// Non-berth upstreams return 404 and the snapshot records "unsupported".
...
// third-party berth relay can live under these domains, while custom
// relays (the only targets that can answer /v1/berth/billing) always do
...
// serve the same role: official APIs that can never host /v1/berth/billing,
...
// 热队列，不再和真正接入 berth 的中转账号抢每周期的探测名额。
// 仍按 upstreamBillingProbeMaxDelay 封顶，保证上游后来接入 berth 时最迟一天
