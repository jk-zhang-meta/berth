...
func TestOpenAIImageOutputCounter_NestedBerth_StandardResponse(t *testing.T) {
	// Simulate what a nested berth setup receives
	// When the upstream berth returns a standard response, the downstream
	// berth receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested berth text-only response, got %d", count)
	}
}
