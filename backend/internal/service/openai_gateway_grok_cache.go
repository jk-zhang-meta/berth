...
const (
	...
	grokClientToolCacheOptInHeader   = "X-Berth-Grok-Client-Tool-Cache"
...
func resolveGrokCacheIdentity(c *gin.Context, body []byte, explicitKey, upstreamModel string) string {
	...
	// upstream session identifiers derived by berth.
...
// the cache-capable route so an intermediate berth does not need to preserve
...
// berth header is consumed locally because buildGrokResponsesRequest only
