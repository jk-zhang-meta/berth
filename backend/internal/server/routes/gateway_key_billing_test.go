...
import (
	...
	servermiddleware "github.com/jk-zhang-meta/berth/internal/server/middleware"
	"github.com/jk-zhang-meta/berth/internal/service"
	"github.com/jk-zhang-meta/berth/internal/web"
...
func TestGatewayRoutesKeyBillingInfoPathIsRegistered(t *testing.T) {
	...
	for _, route := range router.Routes() {
		if route.Method == http.MethodGet && route.Path == "/v1/berth/billing" {
			return
		}
	}

	t.Fatal("GET /v1/berth/billing should be registered")
}
...
func TestGatewayRoutesKeyBillingInfoEndToEnd(t *testing.T) {
	t.Run("missing credentials", func(t *testing.T) {
		...
		router.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/v1/berth/billing", nil))
	...
	t.Run("standard mode", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/v1/berth/billing", nil)
		...
		require.Equal(t, "berth.key_billing", body["object"])
