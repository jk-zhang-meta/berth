...
//
// /v1/usage、/v1/berth/billing 端点与异步生图任务查询只需鉴权，不需要计费执行。
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if errors.Is(err, service.ErrGroupNotAllowed) {
				AbortWithError(c, http.StatusForbidden, "GROUP_NOT_ALLOWED", "Account group access has expired or is not granted")
				return
			}
		...
		billingInfoRequest := c.Request.URL.Path == "/v1/berth/billing"
		// Async image task polling only reads data that already belongs to the
		// authenticated key and must remain available after the completed
		// generation consumes the key's remaining balance.
		skipBilling := c.Request.URL.Path == "/v1/usage" ||
			strings.HasPrefix(c.Request.URL.Path, "/x8Rk3Nq6Vd2/") ||
			billingInfoRequest || isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path)
