...
- `GOCACHE=/tmp/berth-go-cache go test -tags=unit ./... -timeout=5m`：通过，55 个含测试的包成功。
- 最后补充响应格式缓存隔离和冷缓存 304 错误处理后，执行 `GOCACHE=/tmp/berth-go-cache go test -race -tags=unit -p 2 ./internal/service ./internal/handler ./internal/server/routes -run 'Test.*(OpenAIModels|CodexModels|OrdinaryPinned|PinnedModels|ProjectAccountModels)' -count=1 -timeout=3m`：三个包全部通过。
- 最终代码执行 `GOCACHE=/tmp/berth-go-cache GOLANGCI_LINT_CACHE=/tmp/berth-models-lint-cache golangci-lint run ./... --timeout=5m`：0 issues。
