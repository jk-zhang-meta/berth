...
| 项目 | 说明 |
|------|------|
| **上游来源** | Git remote `upstream`（仅用于上游同步） |
| **Fork 仓库** | jk-zhang-meta/berth |
...
| 数据库凭据 | user=`berth`, password=`berth`, dbname=`berth` |
...
psql -U berth -h 127.0.0.1 -d berth -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER berth WITH PASSWORD 'berth';
...
```bash
# 连接数据库
psql -U berth -h 127.0.0.1 -d berth
...
# 执行 SQL 文件
psql -U berth -h 127.0.0.1 -d berth -f migration.sql
```
...
- [上游仓库](https://github.com/jk-zhang-meta/berth)
