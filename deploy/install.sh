#!/bin/bash
#
# Berth Installation Script
# Berth 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/jk-zhang-meta/berth/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="jk-zhang-meta/berth"
INSTALL_DIR="/opt/berth"
SERVICE_NAME="berth"
SERVICE_USER="berth"
CONFIG_DIR="/etc/berth"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="Berth 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Berth 服务："
    ...
    ["not_installed"]="Berth 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Berth。"
    ...
    ["uninstall_complete"]="Berth 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Berth"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Berth"
    ...
    ["server_config_desc"]="配置 Berth 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Berth Installation Script"
    ...
    # Completion
    ["install_complete"]="Berth installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Berth service:"
    ...
    ["not_installed"]="Berth is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Berth from your system."
    ...
    ["uninstall_complete"]="Berth has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Berth"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Berth"
    ...
    ["server_config_desc"]="Configure Berth server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/berth" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/berth" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="berth_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/berth" "$INSTALL_DIR/berth"
    chmod +x "$INSTALL_DIR/berth"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/berth"
}
...
[Unit]
Description=Berth - AI API Gateway Platform
Documentation=https://github.com/jk-zhang-meta/berth
...
User=berth
Group=berth
WorkingDirectory=/opt/berth
ExecStart=/opt/berth/berth
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=berth
...
enable_autostart() {
    ...
    if systemctl enable berth 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status berth"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u berth -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart berth"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop berth"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/berth" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet berth; then
        print_info "$(msg 'stopping_service')"
        systemctl stop berth
    fi

    # Backup current binary
    cp "$INSTALL_DIR/berth" "$INSTALL_DIR/berth.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/berth.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/berth"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start berth

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: Berth must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/berth" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="berth.backup.${current_version}"
        else
            backup_name="berth.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/berth" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop berth 2>/dev/null || true
    systemctl disable berth 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/berth.service
    systemctl daemon-reload
