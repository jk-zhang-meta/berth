# Berth Deployment Files

This directory contains files for deploying Berth on Linux servers and Apple-silicon Macs.
...
| `berth.service` | Systemd service unit file |
| `berth-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete Berth, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
curl -sSL https://raw.githubusercontent.com/jk-zhang-meta/berth/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs berth | grep "admin password"
   ```

### Startup and Database Recovery

Berth applies database migrations during application startup. PostgreSQL can
...
`berth.service`; the application retry covers transient database startup,
while systemd remains the supervisor for permanent process exits. For
Kubernetes, use a PostgreSQL readiness probe and retain the Berth startup
...
- 主进程固定探测 `/tmp/berth-datamanagement.sock`
...
# Restart Berth only
docker compose -f docker-compose.local.yml restart berth
...
| `ADMIN_EMAIL` | No | `admin@berth.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf berth-complete.tar.gz deployment/

# Transfer to new server
scp berth-complete.tar.gz user@new-server:/path/to/destination/
...
Berth supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/jk-zhang-meta/berth/releases)
2. Extract and copy the binary to `/opt/berth/`
3. Copy `berth.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/berth.service
   ```
...
The main config file is at `/etc/berth/config.yaml` (created by Setup Wizard).
...
/opt/berth/
├── berth              # Main binary
├── berth.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 berth
...
# View detailed logs
docker compose logs --tail=100 berth
...
# View recent logs
sudo journalctl -u berth -n 50

# Check config file
sudo cat /etc/berth/config.yaml
...
Berth supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Use a trusted TLS fingerprint collector to capture the device or browser profile, then paste its YAML output into Berth.
