...
Berth can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published Berth, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts Berth.
...
# Restart PostgreSQL, Redis, and Berth in dependency order.
...
The script uses `deploy/.env`, the same source file used by Docker Compose. Export `BERTH_ENV_FILE` to use another file for every command in the current shell:

```bash
export BERTH_ENV_FILE=/absolute/path/to/berth.env
./apple-container.sh init
./apple-container.sh up
```
...
```dotenv
APPLE_CONTAINER_BERTH_IMAGE=ghcr.io/jk-zhang-meta/berth:latest
...
setting only when it creates `berth-apple`. If the existing managed network
...
|---|---|
| Application and gateway variables | Passed to Berth from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and Berth |
...
The script creates only resources carrying the `org.berth.stack=apple-container` label:

| Type | Names |
|---|---|
| Containers | `berth-apple`, `berth-apple-postgres`, `berth-apple-redis` |
| Network | `berth-apple` |
| Volumes | `berth-apple-data`, `berth-apple-postgres-data`, `berth-apple-redis-data` |

The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. Berth and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts Berth. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.

All three services attach only to the private `berth-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `berth-apple-data`.
...
container exec berth-apple sh -c \
...
container exec berth-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
  > backups/berth-data.tar.gz
...
container delete berth-apple
BERTH_IMAGE=ghcr.io/jk-zhang-meta/berth:latest # Match APPLE_CONTAINER_BERTH_IMAGE in .env.
container run --rm --name berth-apple-data-restore \
  --entrypoint /bin/sh \
  --volume berth-apple-data:/restore \
  --volume "$PWD/backups:/backup:ro" \
  "$BERTH_IMAGE" \
  -c 'rm -rf /restore/data && mkdir -p /restore/data && tar -xzf /backup/berth-data.tar.gz -C /restore/data'

# Restore the logical database while the application is absent.
container start berth-apple-postgres
until container exec berth-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/berth.sql berth-apple-postgres:/tmp/berth.sql
container exec berth-apple-postgres sh -c '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  dropdb -h 127.0.0.1 -U "$POSTGRES_USER" --if-exists --force "$POSTGRES_DB"
  createdb -h 127.0.0.1 -U "$POSTGRES_USER" "$POSTGRES_DB"
  psql -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /tmp/berth.sql
  rm /tmp/berth.sql
'
...
- The script targets native `linux/arm64` images. The normal Berth release publishes an arm64 variant.
