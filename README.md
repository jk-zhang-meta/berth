...
Berth is an AI API gateway platform designed to distribute and manage API quotas from AI product subscriptions. Users can access upstream AI services through platform-generated API Keys, while the platform handles authentication, billing, load balancing, and request forwarding.
...
When using Nginx as a reverse proxy for Berth (or CRS) with Codex CLI, add the following to the `http` block in your Nginx configuration:
...
```bash
curl -sSL https://raw.githubusercontent.com/jk-zhang-meta/berth/main/deploy/install.sh | sudo bash
```

The script will:
1. Detect your system architecture
2. Download the latest release
3. Install binary to `/opt/berth`
...
```bash
# 1. Start the service
sudo systemctl start berth

# 2. Enable auto-start on boot
sudo systemctl enable berth
...
```bash
# Check status
sudo systemctl status berth

# View logs
sudo journalctl -u berth -f

# Restart service
sudo systemctl restart berth

# Uninstall
curl -sSL https://raw.githubusercontent.com/jk-zhang-meta/berth/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
mkdir -p berth-deploy && cd berth-deploy

# Download and run deployment preparation script
curl -sSL https://raw.githubusercontent.com/jk-zhang-meta/berth/main/deploy/docker-deploy.sh | bash

# Start services
docker compose up -d

# View logs
docker compose logs -f berth
```
...
git clone https://github.com/jk-zhang-meta/berth.git
cd berth/deploy
...
# 7. View logs
docker compose -f docker-compose.local.yml logs -f berth
```
...
```bash
docker compose -f docker-compose.local.yml logs berth | grep "admin password"
```
...
cd ..
tar czf berth-complete.tar.gz berth-deploy/

# Transfer to new server
scp berth-complete.tar.gz user@new-server:/path/

# On new server
tar xzf berth-complete.tar.gz
cd berth-deploy/
docker compose -f docker-compose.local.yml up -d
```
...
Apple-silicon Macs running macOS 26 can run the full Berth, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
go build -tags embed -ldflags="-X main.Version=${VERSION}" -o berth ./cmd/server
...
- `security.trust_forwarded_ip_for_api_key_acl` enables legacy raw forwarded-header takeover (enabled by default for upgrade compatibility); disable it to enforce `server.trusted_proxies`, which should contain only the exact proxy CIDRs that connect directly to Berth
...
1. **Recommended — let the wizard generate `config.yaml`:** Skip step 5 (do not run the `cp`). Start `./berth` directly; the setup wizard at `http://localhost:8080` walks you through database, Redis, and admin account setup, then writes `config.yaml` for you.
...
2. **If you already created `config.yaml`:** Temporarily move it aside so the wizard can trigger on first run, then restore it afterwards:
   ...
   ./berth        # wizard runs at http://localhost:8080 and writes a fresh config.yaml
   # stop the server (Ctrl+C) once the wizard completes, then restore your config:
   mv config.yaml.bak config.yaml
   ./berth        # restart in normal mode and log in with the admin you just created
   ```
...
Berth supports both Grok subscription accounts through xAI OAuth and standard xAI API-key accounts. Both account types forward OpenAI-compatible Responses traffic to xAI.
...
1. In the Berth admin dashboard, add either a `grok` OAuth account and complete xAI authorization, or add a Grok API-key account.
2. Create a Grok group, attach the account to it, then create a Berth API key assigned to that group.
...
[model."grok"]
model = "grok-4.5"
base_url = "https://your-berth.example.com/v1"
name = "Grok 4.5"
api_key = "sk-your-berth-key"
api_backend = "responses"
context_window = 1000000
supports_backend_search = true
```

Back up an existing `config.toml` before merging the entry. The file contains a Berth API key, so keep it private and restrict its permissions where supported. Verify the effective configuration and make a smoke request:

```bash
grok inspect
grok -p "Reply with berth-ok" -m grok
```

The `base_url` above is the public Berth URL ending in `/v1`, not `api.x.ai` or the internal xAI OAuth proxy URL.

### Usage And Quota Display

xAI quota is passive. Berth does not invent subscription quota values; it records whitelisted xAI rate-limit headers from successful or rate-limited upstream responses when xAI sends them. Before the first usable upstream response, the dashboard shows quota as unknown and still displays local Berth usage stats.
...
Berth supports [Antigravity](https://antigravity.so/) accounts. After authorization, dedicated endpoints are available for Claude and Gemini models.
...
<a href="https://star-history.dera.page/#jk-zhang-meta/berth&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=jk-zhang-meta/berth&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=jk-zhang-meta/berth&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=jk-zhang-meta/berth&type=Date" />
 </picture>
</a>
