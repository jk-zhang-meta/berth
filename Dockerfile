...
RUN --mount=type=cache,id=berth-pnpm-store,target=/root/.local/share/pnpm/store \
...
RUN --mount=type=cache,id=berth-gomod,target=/go/pkg/mod \
    go mod download
...
RUN --mount=type=cache,id=berth-gomod,target=/go/pkg/mod \
    --mount=type=cache,id=berth-gobuild,target=/root/.cache/go-build \
...
# Labels
LABEL maintainer="Berth Maintainers"
LABEL description="Berth - AI API Gateway Platform"
LABEL org.opencontainers.image.source="https://github.com/jk-zhang-meta/berth"
...
RUN addgroup -g 1000 berth && \
    adduser -u 1000 -G berth -s /bin/sh -D berth

# Set working directory
WORKDIR /app

# Copy binary/resources with ownership to avoid extra full-layer chown copy
COPY --from=backend-builder --chown=berth:berth /app/berth /app/berth
COPY --from=backend-builder --chown=berth:berth /app/backend/resources /app/resources

# Create data directory
RUN mkdir -p /app/data && chown berth:berth /app/data

# Copy entrypoint script (fixes volume permissions then drops to berth)
...
# Run the application (entrypoint fixes /app/data ownership then execs as berth)
